...
- 当前版本：**Ver2.9-rc17**（库存类型阶段 2 低风险收口，开发版未同步正式 App）
- 运行平台：Windows
- 服务端口：`8787`
...
- **库存汇总**：库存类型/库存状态/是否库存物料筛选；列顺序调整；参考 CSV 导出（不参与导入）
- **物料管理**：列表展示是否成品设备、是否维修备件、成本方式；库存属性说明文案
- **出入库提示**：采购入库成品设备提醒；销售/生产库存不足提醒不拦截；成品入库须 BOM/机型成本
- 本阶段未重构库存核心计算，未改应收/应付/财务

## Ver2.9-rc16（已完成）

- tag `v2.9.0-rc16`；已 commit/push、正式 App 已同步、8787 验收通过
- **物料库存类型字段**：StockType、是否库存物料、安全库存、默认仓库、成本方式等
- **库存汇总**：展示库存类型、安全库存、库存状态
- **出入库表单**：轻量带出库存类型；库存不足提醒但不拦截
