# ERP Ver2.9-rc18-rc19 table display + document flow pack script
...
$PackLabel = "ERP_Ver29_rc18_rc19" + "表格与单据流转成果给ChatGPT"
$LogDir = Join-Path $Desktop ($PackLabel + "_" + $Timestamp)
$LogFile = Join-Path $LogDir ("pack_rc18_rc19_log_" + $Timestamp + ".txt")
$SummaryFile = Join-Path $LogDir ("pack_rc18_rc19_summary_" + $Timestamp + ".md")
...
try {
    ...
    $DocFiles = @(
        ...
        "tools\erp-agent\scripts\pack_rc18_rc19_table_flow_polish.ps1"
    )
    ...
    if (Test-Path $reportDir) {
        Get-ChildItem -Path $reportDir -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -like "ver29_regression_summary_*" -or $_.Name -like "ver29_pages_check_*" -or $_.Name -like "*rc18*" -or $_.Name -like "*rc19*" -or $_.Name -like "*rc17*" } |
    ...
    $summaryText = "# ERP Ver2.9-rc18-rc19 pack summary`nTime: $Timestamp`nOverall: $overall`nBuild: $buildResult`nHTTP: $BaseUrl $httpResult`nLogDir: $LogDir`nZip: $ZipFile`nScope: table display + sales outbound + purchase inbound flow`nBoundary: no commit/push/tag/sync/clear/006"
    ...
    if ($FilesToZip.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "rc18-rc19 pack done" -ForegroundColor Green
