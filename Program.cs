...
namespace SupplierErpApp
{
    ...
    public class SalesOrder
    {
        ...
        public string UpdatedAt { get; set; }
        public string UpdatedBy { get; set; }
        public List<SalesOrderLine> Items { get; set; }
    }
    ...
    public static partial class Program
    {
        ...
        static string BackupDir = Path.Combine(AppRoot, "Backups");
        ...
        {
            BackupDir = IsDefaultDataDirectory ? Path.Combine(AppRoot, "Backups") : Path.Combine(DataDir, "backups");
        ...
        static void Handle(HttpListenerContext ctx)
        {
            ...
            try
            {
                ...
                if (!ValidateUnsafeRequestOrigin(ctx)) return;
                ...
                if (path == "/api/sales-orders" && ctx.Request.HttpMethod == "GET") { if (!RequirePermission(ctx, user, "sales_order.view")) return; WriteJson(ctx, LoadSalesOrdersForResponse()); return; }
        ...
        static void ResolveSalesOrderLink(SalesOutbound item, SalesOrder knownOrder = null)
        {
            ...
            item.ItemType = NormalizeSalesItemType(line.ItemType);
            item.ModelCostId = line.ModelCostId ?? "";
            item.BomId = line.BomId ?? "";
            if (item.ItemType == "FinishedProduct")
            {
                ...
                else
                {
                    ...
                    item.MaterialName = line.MaterialName ?? "";
                }
                item.MaterialId = "";
            }
            else
        ...
        static void ResolvePurchaseOrderLink(PurchaseInbound item, PurchaseOrder knownOrder = null)
        {
            ...
            if (knownOrder != null)
            {
                var line = SelectPurchaseOrderLineForInbound(knownOrder, item);
                ...
                item.MaterialCode = line.MaterialCode ?? "";
                item.MaterialName = line.MaterialName ?? "";
                if (item.InboundPrice <= 0) item.InboundPrice = line.UnitPrice;
                return;
            }
        ...
        {
            var list = LoadJsonList<SalesOrder>(SalesOrdersFile);
            foreach (var item in list) EnsureSalesOrderItemsForRead(item);
            return list;
        }
        static List<SalesOrder> LoadSalesOrdersForResponse()
        {
        ...
        static void ApplySalesOrder(SalesOrder item)
        {
            ...
            NormalizeSalesOrderItemsForSave(item);
        ...
        static void UpdateSalesOutbound(HttpListenerContext ctx, UserSession user, string id)
        {
            ...
            var saved = MutateJsonList<SalesOutbound, SalesOutbound>(SalesOutboundsFile, "sales_outbounds", list =>
            {
                ...
                item.SalesOrderId = input.SalesOrderId; item.SalesOrderLineId = input.SalesOrderLineId; item.SalesOrderNo = input.SalesOrderNo;
        ...
        {
            var list = LoadJsonList<PurchaseOrder>(PurchaseOrdersFile);
            foreach (var item in list) EnsurePurchaseOrderItemsForRead(item);
            return list;
        }
        ...
        static void ApplyPurchaseOrder(PurchaseOrder item)
        {
            ...
            NormalizePurchaseOrderItemsForSave(item);
            item.OrderDate = string.IsNullOrWhiteSpace(item.OrderDate) ? TodayText() : item.OrderDate.Trim();
            item.Status = NormalizeDocStatus(item.Status);
            item.Note = (item.Note ?? "").Trim();
        }
        ...
        static void UpdatePurchaseInbound(HttpListenerContext ctx, UserSession user, string id)
        {
            ...
            var saved = MutateJsonList<PurchaseInbound, PurchaseInbound>(PurchaseInboundsFile, "purchase_inbounds", list =>
            {
                ...
                if (IsConfirmedStatus(item.Status) && (item.Quantity != input.Quantity || !string.Equals(item.MaterialId ?? "", input.MaterialId ?? "", StringComparison.Ordinal)))
                    BizFail(ReferenceLockMessage, 409);
                item.PurchaseOrderId = input.PurchaseOrderId; item.PurchaseOrderLineId = input.PurchaseOrderLineId; item.PurchaseNo = input.PurchaseNo;
        ...
        {
            var sb = new StringBuilder(); sb.AppendLine("订单编号,订单日期,客户编号,客户名称,物料编号,物料名称,数量,不含税销售单价,含税销售单价,不含税销售金额,含税销售金额,状态,备注,明细JSON,最后更新,操作人");
            foreach (var x in LoadSalesOrders()) sb.AppendLine(string.Join(",", new[] { x.Code, x.OrderDate, x.CustomerCode, x.CustomerName, x.MaterialCode, x.MaterialName, x.Quantity.ToString("0.##"), x.TaxExcludedSalePrice.ToString("0.00"), x.TaxIncludedSalePrice.ToString("0.00"), x.TaxExcludedSaleAmount.ToString("0.00"), x.TaxIncludedSaleAmount.ToString("0.00"), x.Status, x.Note, Json.Serialize(x.Items ?? new List<SalesOrderLine>()), x.UpdatedAt, x.UpdatedBy }.Select(Csv)));
        ...
        {
            var sb = new StringBuilder(); sb.AppendLine("采购编号,订单日期,供应商名称,物料编号,物料名称,数量,采购单价,采购金额,状态,备注,明细JSON,最后更新,操作人");
            foreach (var x in LoadPurchaseOrders()) sb.AppendLine(string.Join(",", new[] { x.Code, x.OrderDate, x.SupplierName, x.MaterialCode, x.MaterialName, x.Quantity.ToString("0.##"), x.UnitPrice.ToString("0.00"), x.Amount.ToString("0.00"), x.Status, x.Note, Json.Serialize(x.Items ?? new List<PurchaseOrderLine>()), x.UpdatedAt, x.UpdatedBy }.Select(Csv)));
        ...
        static void StockAdd(Dictionary<string, StockAgg> map, string itemType, string itemId, string itemCode, string itemName, string spec, string unit, decimal qty, decimal unitCost)
        {
            ...
            agg.Quantity += qty;
            if (unitCost >= 0) agg.CostAmount += CalcLineAmount(qty, unitCost);
            map[key] = agg;
        }
        ...
        static List<StockItem> BuildStockItems()
        {
            return BuildStockMap().Values.Select(agg =>
            {
                ...
                decimal costPrice = qty == 0 ? 0 : RoundMoney(agg.CostAmount / qty);
                if (costPrice < 0) costPrice = Math.Abs(costPrice);
                ...
                return new StockItem
                {
                    ...
                    CurrentQuantity = qty,
                    CostPrice = costPrice,
                    StockAmount = RoundMoney(qty * costPrice),
                    StockType = stockType,
        ...
        static bool SameRequestOrigin(HttpListenerRequest req, Uri candidate)
        {
            Uri requestUrl = req != null ? req.Url : null;
            if (requestUrl == null || candidate == null) return true;
            return string.Equals(requestUrl.Scheme, candidate.Scheme, StringComparison.OrdinalIgnoreCase)
                && string.Equals(requestUrl.Host, candidate.Host, StringComparison.OrdinalIgnoreCase)
                && EffectivePort(requestUrl) == EffectivePort(candidate);
        }
        static bool ValidateUnsafeRequestOrigin(HttpListenerContext ctx)
        {
            if (ctx == null || !IsUnsafeHttpMethod(ctx.Request.HttpMethod)) return true;
            string origin = ctx.Request.Headers["Origin"];
            Uri originUri;
            if (!string.IsNullOrWhiteSpace(origin))
            {
                if (!Uri.TryCreate(origin, UriKind.Absolute, out originUri) || !SameRequestOrigin(ctx.Request, originUri))
                {
                    WriteJson(ctx, new { error = "跨站请求被拒绝" }, 403);
                    return false;
                }
            }
            ...
            {
                if (!Uri.TryCreate(referer, UriKind.Absolute, out refererUri) || !SameRequestOrigin(ctx.Request, refererUri))
                {
                    WriteJson(ctx, new { error = "跨站请求被拒绝" }, 403);
                    return false;
                }
